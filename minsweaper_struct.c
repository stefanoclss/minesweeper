...
#include <stdlib.h>
#include <time.h>
#define HORIZONTAL_CELS 6
#define VERTICAL_CELS 6
#define TOTAL_BOMBS 10
/*
 *i, j values for boundaries for loop ,  *pre_for_loop pointer to function to use inside function
...
void print_grid(struct Cell *ptr_field , int actual)    //print the grid with double for loop
{
	...
	for (int i=0; i<VERTICAL_CELS; i++)
	{
		...
		for (int j=0; j< HORIZONTAL_CELS ; j++)
		{
			...
			{
				*(ptr_field + find(j, i))->vissible_value = 'q';				//print grid as shown in game
...
int main(void)
{
	srand(time(NULL));                                             // maakt het mogelijk om random te gebruiken
	...
	printf("\n choose an y coordinate \n");
	scanf("%d"  , &y);
	place_mines(x, y,&field);    //zal eerste mijnen plaatsten niet op de plek van de eerste move die we juist hebben gekozen
